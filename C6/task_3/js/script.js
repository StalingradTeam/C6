...
const error = () => {
    ...
    const errTxt = 'Нет доступа к местоположению'
...
const success = (position) => {
    ...
    let textValue = `<a class="map-link" target="_blank" href = 'https://www.openstreetmap.org/#map=18/${latitude}/${longitude}'>GEO-локация</a>`
...
btnLocation.addEventListener('click', () =>{
    ...
    if (!navigator.geolocation) {
        status.textContent = 'Geolocation не поддерживается браузером';
  } else {
